...
**`ai.js`** (same folder) — Ollama-driven conversation + action loop, local model `qwen2.5:14b` (tool-calling capable), talked to via `/api/chat`.
...
- Everything else goes to the model with a small tool set: `come_here`, `follow_player <name>`, `stop`, `equip_item`, `give_item <name> <item> [count]` (paths to the player and `bot.toss()`s the item — needs `bot.players[name].entity` to exist, i.e. the target must actually be loaded/visible to the bot's client, or it replies "I can't see them"), and `wiki_lookup <topic>` (queries the real `minecraft.wiki` MediaWiki API for search + plain-text extract, then feeds the result back to the model for a natural reply — no trust gate, it's read-only).
...
- The owner-whisper for a pending request used to fail silently (`try { bot.whisper(...) } catch {}` with an empty catch) — now logged to the bot's console/log if it ever fails again, instead of just looking like the bot never asked.
- **Ollama/GPU gotcha:** at the default 32K context, `qwen2.5:14b`'s weights+KV-cache don't fit in the RX 6700 XT's 12GB VRAM and Ollama silently splits ~32% of layers to CPU (`ollama ps` shows this as e.g. `32%/68% CPU/GPU` — much slower). Every call here passes `options: { num_ctx: 4096 }` (`OLLAMA_NUM_CTX` env var) to force `100% GPU`; conversations here are short enough that this loses nothing. Check with `ollama ps` right after a call if this ever regresses.
...
- Right-click a bot account, or `/botinv <name>`, opens its **main inventory** (36 slots, `BotInventoryHolder`) — but only for the owner or a friend (see trust below); anyone else gets the trade screen instead.
