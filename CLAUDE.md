...
- **Auto-eat chest fallback (2026-09-07):** below 70% hunger, checks inventory first, then nearby chests/barrels (24 blocks, via new `/whatsin <x> <y> <z>` — same ground-truth-via-Bukkit reasoning as `/botinv list`, generalized to any container block), and only notifies the owner (still rate-limited) if neither has food. `findFoodInNearbyChests()`/`eatFromChest()` in `ai.js`.
...
- **Major bug found 2026-09-06/07: the bot's own client-side ENTITY tracking is also unreliable, not just item data.** Confirmed live: a player only ~12 blocks away never appeared in `bot.entities` or `bot.players[x].entity` at all — the same general class of bug as the item-id mismatch (partial-packet parse warnings show up for `entity_metadata` too, not just login), just affecting entity-spawn packets instead of item decoding. This silently broke `come_here`, `follow_player`, `give_item`, and the friend-position context. **Fix:** added `/whereis <player>` to BotInteropPlugin (server-side, Bukkit-authoritative, not restricted to bot accounts like `/botstatus`) — `queryPlayerPosition()` in `ai.js` self-issues it and uses the result directly instead of trusting `bot.entities`. `follow_player` can't use pathfinder's entity-based `GoalFollow` any more (no reliable entity reference) — it now polls the real position every 2s and reissues `GoalNear`. `findPlayerEntity()` was removed entirely — nothing uses it, and it was fundamentally broken for players. If a *mob*-targeting feature (hunting, self-defense) ever seems similarly broken, suspect the same root cause — not yet confirmed either way for non-player entities.
- **`fish` gotcha, fixed:** `bot.fish()` just activates the held item facing wherever the bot currently looks — casting without explicitly facing water first hooked whoever was standing in front of the bot instead (confirmed live). Now finds and faces nearby water before casting, and loops several catches per call (capped, cancellable via `stop`) so "fish until I say stop" doesn't need re-triggering every message.
- **Second `fish` bug, deeper one:** `bot.fish()`'s bite detection depends on parsing `world_particles` packets, and those fail to decode on this server — confirmed via 1400+ repeated `PartialReadError`s reading the `Particle` variant while the bot was fishing (real 26.2's packet structure doesn't match this patched/26.1-based protocol schema, same class of bug as the item-id and entity-tracking issues above). That left `bot.fish()` hanging forever on the very first cast. **Real fix (not just a timer guess):** the `fishing_bobber` entity has its own `biting` metadata field (`bot.registry.entitiesByName.fishing_bobber.metadataKeys`, index 9) — a plain boolean over `entity_metadata`, structurally simpler than the broken particle payload, and not affected by the same decode bug in testing. `trackBobberSpawn()`/`waitForBite()` in `ai.js` track the real bobber entity and its `biting` flag directly, reeling in the instant it goes true — falls back to a fixed 7s wait only if the bobber's own spawn packet is never matched.
- **Multi-tool-call responses were silently dropping everything after the first**, confirmed live: "go to X then resume fishing" correctly came back as two `tool_calls` from the model, but only the first ever ran. `handleAiMessage` now loops over every tool call in one response instead of only using `tool_calls[0]`.
- Added `goto_location(x, y, z)` — exact-coordinate navigation, for when conversational/landmark references aren't enough.
- Added `turn(degrees)` (2026-09-07) — "turn 90 degrees clockwise" had no matching tool at all, which looked like the bot ignoring/rejecting commands but was really just a missing capability. Positive = clockwise/right, matching Minecraft/mineflayer's yaw convention (south=0 -> west=90 -> north=180 -> east=270).
- **Block-type corruption confirmed too (2026-09-07), not just items/entities:** searching for a furnace/smoker with `bot.findBlock()` actually matched a real campfire instead, so `bot.openFurnace()` hung 20s waiting for a GUI window a campfire doesn't have. Added `/findblock <material1,material2,...> [radius]` to BotInteropPlugin — a fast, radius-bounded (16 blocks horizontal, 6 vertical, small enough to stay synchronous) real Bukkit-side scan. `queryBlockPosition()` in `ai.js` uses this instead of trusting `bot.findBlock`'s own type matching for both `smelt` and `use_campfire`; `bot.blockAt(pos)` is still used to get an interactable Block, but only as an opaque position handle, same principle as the item fixes.
- **`use_campfire` added, then found to silently fail (2026-09-07):** placement/collection both trusted client-side calls that resolve successfully even when the server silently rejects them — confirmed live the bot claimed "put 4 salmon on the campfire" and "collected the cooked salmon" while the real raw-salmon count never dropped and no cooked_salmon ever existed. Fixed with `countMatchingItems()` (ground-truth count via `/botinv list`, alongside `findItemByRealName`): placement only counts if the real item count actually decreases after each `activateBlock()` call, and collection only claims success if the real cooked-item name (via a raw->cooked map — most are `cooked_<name>`, but potato/kelp are `baked_potato`/`dried_kelp` exceptions) actually shows up afterward. Broadened to the full wiki-confirmed cookable list (beef, chicken, rabbit, porkchop, mutton, cod, salmon, potato, kelp — not just fish), with `smelt` explicitly deferring to it for campfire-specific requests.
- **Multi-tool-call responses and malformed pseudo-tool-call text (2026-09-07):** `handleAiMessage` now loops over every tool call in one response instead of only using `tool_calls[0]` (confirmed live: "go to X then resume fishing" correctly came back as two separate tool_calls, but only the first ever ran). Separately, under the full ~18-tool live context, the model sometimes writes its tool call as plain text instead of a real `tool_calls` entry (e.g. `smelt {"item": "raw fish"}`) — confirmed this is a real context-size-dependent reliability limit, not fixable by prompting alone (an isolated 2-tool test with identical phrasing works every time). `parseRawToolCallText()` recovers and executes the model's actual intent instead of discarding it (an earlier version just suppressed/hid this text, which traded a visible glitch for silent, invisible failure — worse).
- **`trustedPeopleContext(bot)` (2026-09-06):** every call also names the owner explicitly and lists each friend's live position (via the same `findPlayerEntity` used by `come_here`/`follow_player`) — before this, the model had no fact for who its owner even was (the earlier "I am your owner" echo bug was partly this) and couldn't answer "where am I"/"where is `<friend>`" without guessing. `findPlayerEntity()` is now case-insensitive throughout, since OWNER/friends are stored lowercase but real Minecraft usernames aren't.
- **Survival skills (2026-09-06/07):** `mine_block`, `craft_item`, `smelt` (furnace/smoker/blast_furnace), `farm` (harvest + replant wheat/carrots/potatoes/beetroot), `fish` (wraps mineflayer's built-in `bot.fish()`), `brew_potion` (brewing stand, fixed slot layout: bottles 0-2, ingredient 3, fuel 4, via `bot.transfer`), `enter_boat`/`exit_boat`, `attack_nearby_hostile`. Ladders needed no code — `Movements` already treats climbable blocks as safe. `findItemByRealName()` generalizes the eat-bug ground-truth workaround (self-issues `/botinv list`, matches a Bukkit-verified real name, returns whatever object sits at the matching protocol slot) — every new tool that needs to locate an item uses this instead of searching `bot.inventory` by its own name. **Caveat:** `craft_item`/`smelt`/`brew_potion` still depend on mineflayer's own recipe/slot-type matching, which goes through the same locally-patched item table as the eat bug — self-consistent internally, but could diverge from what the real server does with a given recipe; not verified bulletproof the way `eat` now is. `attachAutoSurvival()` also runs a throttled nearby-hostile scan and fights back on its own. `surroundingsContext(bot)` (cached 5s) lists notable nearby blocks (ores, logs, water, lava, crafting table, furnace, brewing stand, farmland) within 16 blocks — block names are far more version-stable than items, so this didn't need the ground-truth workaround. **Explicitly out of scope:** autonomous redstone/hopper circuit design — no existing library does general-purpose contraption planning; `wiki_lookup` still covers explaining how redstone/hoppers work.
- **Pathing hardened (2026-09-06):** `canOpenDoors=true` (was false), `maxDropDown` 4→3 (vanilla's exact no-fall-damage threshold), `liquidCost` 1→5 (strongly prefers a dry path, only swims when there's no alternative). Lava was already in `Movements`' own default `blocksToAvoid` — no change needed there. Parkour/sprinting left at their already-true defaults so jumping still works normally.
...
- `FinderTools` (2026-09-07) — third-party plugin from Modrinth (https://modrinth.com/plugin/findertools), installed for mining/ore-location awareness. Declares `api-version: '1.20'` and hasn't been updated in ~11 months, but the jar's bytecode only references stable public Bukkit API (`Bukkit`, `Chunk`, `Material`, `Block`, `Entity`, `Player`, etc. — no NMS/reflection), and it loads and runs correctly on this server despite the large version gap — confirmed live with an actual `/findertools find block diamond_ore world` scan. Commands: `/findertools find <block|entity> <id> <world> [options]`, `stop`, `pause`, `start`/`resume`, `status`, `reload`. A full-world scan is genuinely slow (loaded-chunks-only test showed ~5h42m ETA for one ore type) and competes with Chunky/BlueMap for CPU — don't leave a scan running unattended without knowing that.
...
**World pregeneration (2026-09-06):** Chunky is pregenerating a 10,000-block-radius square on `world` centered on spawn (`chunky world world; chunky spawn; chunky shape square; chunky radius 10000; chunky start/confirm`), and BlueMap auto-renders newly generated chunks as they land (no manual trigger needed). This is a genuinely long-running task (millions of chunks) that competes with normal gameplay for CPU on this 4-core box — expect it to take many hours and cause some lag while it runs. It does **not** survive a server restart; run `/chunky continue` afterward or it just sits paused (this happened several times during 2026-09-06/07's session — re-run it after every Java plugin deploy). Check progress with `/chunky progress`. As of 2026-09-07 ~03:05 it's at 96.8%, ETA ~45 min.

**BlueMap render-thread-count** started at 1 (the shipped default), bumped to 2 then 3 over the course of 2026-09-06/07 as Chunky's pregeneration approached completion and stopped competing for the same 4 cores — set in `plugins/BlueMap/core.conf`, applied via `/bluemap reload` (no restart needed). At 3 threads overnight (2026-09-07), with Chunky about to finish and no players expected online, BlueMap's own render (separate from chunk generation — this is turning generated chunks into map tiles) was at 23% with a ~15 hour ETA at 2 threads. Don't go to 4 (all cores) without leaving headroom for the main server tick thread — that's the difference between "slow" and an actual TPS-crash/watchdog kill.
