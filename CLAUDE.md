...
- **Second `fish` bug, deeper one:** `bot.fish()`'s bite detection depends on parsing `world_particles` packets, and those fail to decode on this server — confirmed via 1400+ repeated `PartialReadError`s reading the `Particle` variant while the bot was fishing (real 26.2's packet structure doesn't match this patched/26.1-based protocol schema, same class of bug as the item-id and entity-tracking issues above). That left `bot.fish()` hanging forever on the very first cast. Replaced with `fishOnce()`: cast, wait a fixed window (`FISH_CAST_WAIT_MS`), reel in regardless — lower catch rate than real bite-timing, but it actually completes.
